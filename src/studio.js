...
const TEMPLATES=[
 {id:'cream',name:'Clinic / cream fade',tag:'The conversation',headline:'YOU TALK.\nWE LISTEN.',body:'Your routine, your goals, and your questions. Start with a conversation and a full evaluation.',photo:'clinic-slide-03',callout:'Schedule a consultation'},
 {id:'navyfade',name:'Photo / navy fade',tag:'Men’s Wellness Centers',headline:'MAKE ROOM\nFOR YOUR HEALTH.',body:'Personal goals. A full evaluation. Care built around you.',photo:'clinic-slide-06',callout:'Schedule a consultation'},
 {id:'frame',name:'Cream / bold frame',tag:'Your next step',headline:'A CARE PLAN\nFOR EVERYDAY LIFE.',body:'Daily routines and personal goals belong in the care conversation.',callout:'Schedule a consultation'},
 {id:'focus',name:'Navy / headline focus',tag:'Start with a conversation',headline:'YOUR QUESTIONS.\nTIME TO TALK.',body:'Bring the things you want to understand about your health. We start by listening.',callout:'Schedule a consultation'},
 {id:'cliniccard',name:'Clinic / photo window',tag:'A look inside MWC',headline:'CARE STARTS\nWITH A VISIT.',body:'A conversation about your health, in person. Make time for your questions.',photo:'clinic-slide-05',callout:'Schedule a consultation'},
...
const PHOTO_LAYOUTS=['split','photo','cream','navyfade','cliniccard'];
let active='cream',images={},logos={},ready=false,grid=false,timer,drafts={};
function initial(t){return {format:'portrait',eyebrow:t.tag,headline:t.headline,accent:t.accent||'',body:t.body||'',callout:t.callout||'',benefits:t.benefits||'',cta:['cream','navyfade','frame','focus','cliniccard'].includes(t.id)?'menswellnesscenters.com':'BOOKMWC.COM',footer:'',position:t.id==='split'?'100':t.id==='photo'?'70':'50',darkness:'18',color:'#ee6b06',scale:'1',photoKey:t.photo||'fathers-day-fd-slide-1-beach',custom:null}}
TEMPLATES.forEach(t=>drafts[t.id]=initial(t));
function state(){return {...drafts[active],...Object.fromEntries(keys.map(k=>[k,$(k).value]))}}
function loadFields(){keys.forEach(k=>$(k).value=drafts[active][k]);const relevant={accent:['myth','number','comparison'],benefits:['checklist','number','comparison','steps'],callout:['editorial','quote','appointment','cream','navyfade','frame','focus','cliniccard']};for(const [k,ids] of Object.entries(relevant))$(k).closest('.field').classList.toggle('hidden',!ids.includes(active));const isPhoto=PHOTO_LAYOUTS.includes(active);$('position').closest('.field').classList.toggle('hidden',!isPhoto);$('darkness').closest('.field').classList.toggle('hidden',!['photo','navyfade'].includes(active));$('photos').style.opacity=isPhoto?'1':'.5';$('body').closest('.field').classList.toggle('hidden',['quote','number'].includes(active));$('listHint').textContent=({checklist:4,number:3,comparison:4,steps:3}[active]||3)+' maximum; separate with |';$('accent').closest('.field').firstChild.textContent=active==='number'?'Number ':active==='comparison'?'Column labels (separate with |) ':'Replacement text '}
...
function paint(target,id,s){
 ...
 const dark=['statement','quote','question','photo','steps','navyfade','focus'].includes(id);
 ...
 function font(size,style){const f={bold:'700 SIZEpx Arial, sans-serif',plain:'SIZEpx Arial, sans-serif',display:'700 SIZEpx Oswald, sans-serif',condensed:'SIZEpx Impact, "Arial Narrow", sans-serif',note:'700 SIZEpx "Courier New", monospace',hand:'SIZEpx "Segoe Print", "Comic Sans MS", cursive'};c.font=(f[style]||f.plain).replace('SIZE',size)}
 ...
 if(['cream','navyfade','frame','focus','cliniccard'].includes(id)){
  const fg=dark?white:ink;
  rect(0,0,1080,H,dark?ink:'#f5f3ed');
  if(id==='cream'||id==='navyfade'){
   photo(0,0,1080,Y(.57));
   if(id==='navyfade')rect(0,0,1080,Y(.57),`rgba(11,16,40,${s.darkness/100})`);
   const g=c.createLinearGradient(0,Y(.16),0,Y(.57));
   g.addColorStop(0,dark?'rgba(11,16,40,0)':'rgba(245,243,237,0)');g.addColorStop(1,dark?ink:'#f5f3ed');
   c.fillStyle=g;c.fillRect(0,Y(.16),1080,Y(.57)-Y(.16));
   round(X(-.01),Y(0)-7,W(.30),V(.082),dark?ink:'#f5f3ed',6);logo(0,.006,.27,dark);
   T(s.eyebrow,0,.43,1,.04,25,'plain',fg);
   T(s.headline,0,.49,1,.22,98,'display',fg);
   T(s.body,0,.725,1,.115,32,'plain',fg);
  }else if(id==='cliniccard'){
   logo(0,0,.29,false);T(s.eyebrow,.39,.01,.61,.05,25,'plain',ink,'right');
   photo(X(0),Y(.115),W(1),V(.33));
   T(s.headline,0,.485,1,.22,94,'display',ink);T(s.body,0,.73,1,.10,32,'plain',ink);
  }else{
   if(id==='frame'){c.strokeStyle=ink;c.lineWidth=4;c.beginPath();c.roundRect(42,z.y-55,996,z.h+108,48);c.stroke()}
   logo(0,.005,.31,dark);T(s.eyebrow,0,.16,1,.05,29,'plain',fg);
   T(s.headline,0,.27,1,.34,112,'display',fg);
   rect(X(0),Y(.635),W(.40),7,A);T(s.body,0,.69,1,.14,35,'plain',fg);
  }
  round(X(0),Y(.865),W(.79),V(.073),A,30);T(s.callout,.028,.881,.735,.044,29,'bold',ink,'center');
  T(s.cta,0,.962,1,.036,25,'plain',fg);
 }
...
function setGrid(v){grid=v;$('collection').classList.toggle('hidden',!v);$('canvasWrap').style.display=v?'none':'block';$('guideControls').hidden=v;document.querySelector('.stage').classList.toggle('gridmode',v);$('singleView').classList.toggle('active',!v);$('gridView').classList.toggle('active',v);$('previewNote').textContent=v?'Your template collection • Click a post to edit':'Instagram preview • Export matches this canvas';$('export').textContent=v?'↓ Export grid PNG':'↓ Export PNG';if(v)drawGrid()}
...
$('export').onclick=()=>{render();const checks=grid?[...$('collection').children].map(b=>b.firstChild):[canvas];if(checks.some(c=>c._warnings.length))return toast('Shorten flagged copy or reduce list items before exporting.');let out=canvas;if(grid){out=document.createElement('canvas');out.width=3240;out.height=Math.ceil(TEMPLATES.length/3)*1350;const c=out.getContext('2d');[...$('collection').children].forEach((b,i)=>c.drawImage(b.firstChild,(i%3)*1080,Math.floor(i/3)*1350))}out.toBlob(b=>{if(!b)return toast('Export failed. Please try again.');download(b,grid?'MWC-template-grid.png':`MWC-${active}-${$('format').value}.png`);toast('Full-resolution PNG exported.')},'image/png')};
$('save').onclick=()=>{drafts[active]=state();download(new Blob([JSON.stringify({version:2,active,drafts})],{type:'application/json'}),'MWC-studio-project.json');toast('All template drafts saved.')};$('load').onclick=()=>$('project').click();
$('project').onchange=async e=>{let f=e.target.files[0];if(!f)return;try{const p=JSON.parse(await f.text());if(p.version!==2||!TEMPLATES.some(t=>t.id===p.active))throw Error();let clean={};for(const t of TEMPLATES){const d=p.drafts[t.id]||initial(t);if(keys.some(k=>typeof d[k]!=='string')||!['portrait','square','story'].includes(d.format)||!['1','0.85','1.12'].includes(d.scale)||!/^#[0-9a-f]{6}$/i.test(d.color)||!Object.hasOwn(ASSETS,d.photoKey)||d.photoKey==='brand'||!Number.isFinite(+d.position)||+d.position<0||+d.position>100||!Number.isFinite(+d.darkness)||+d.darkness<0||+d.darkness>65)throw Error();clean[t.id]={...initial(t),...Object.fromEntries(keys.map(k=>[k,d[k].slice(0,$(k).maxLength>0?$(k).maxLength:1000)])),photoKey:d.photoKey,custom:null};if(d.custom){if(typeof d.custom!=='string'||!/^data:image\/(png|jpeg|webp);base64,/.test(d.custom))throw Error();images[d.custom]=await loadImage(d.custom);clean[t.id].custom=d.custom}}drafts=clean;active=p.active;loadFields();render();toast('Your collection is open.')}catch{toast('Choose a project saved by this studio.')}e.target.value=''};
$('upload').onchange=async e=>{const f=e.target.files[0];if(!f)return;if(!['image/jpeg','image/png','image/webp'].includes(f.type)||f.size>25*1024*1024)return toast('Choose a JPG, PNG or WebP under 25 MB.');try{const src=await new Promise((resolve,reject)=>{const r=new FileReader;r.onload=()=>resolve(r.result);r.onerror=reject;r.readAsDataURL(f)});images[src]=await loadImage(src);if(!PHOTO_LAYOUTS.includes(active))select(['frame','focus'].includes(active)?'cream':'split');drafts[active].custom=src;render();toast('Your supplied photo is ready.')}catch{toast('Image could not be opened. Try another file.')}e.target.value=''};
$('reset').onclick=()=>{drafts[active]=initial(TEMPLATES.find(t=>t.id===active));loadFields();render();toast('This template has been reset.')};
async function init(){try{await Promise.all(Object.entries(ASSETS).map(async([k,v])=>images[k]=await loadImage(v)));brandAssets();logos.dark=await loadImage(BRAND.dark);logos.light=await loadImage(BRAND.light);await document.fonts.load('700 96px Oswald');TEMPLATES.forEach(t=>{const b=document.createElement('button');b.className='layout';b.dataset.layout=t.id;b.setAttribute('aria-label',t.name);const thumb=document.createElement('canvas');paint(thumb,t.id,initial(t));const name=document.createElement('b');name.textContent=t.name;b.append(thumb,name);b.onclick=()=>select(t.id);$('templates').append(b);const g=document.createElement('button');g.dataset.id=t.id;g.title=t.name;g.setAttribute('aria-label','Edit '+t.name);g.append(document.createElement('canvas'));g.onclick=()=>select(t.id);$('collection').append(g)});Object.keys(ASSETS).filter(k=>k!=='brand').forEach((k,i)=>{let b=document.createElement('button');b.className='photo';b.dataset.key=k;b.title=(PHOTO_LABELS[k]||photoNames[i]||({'clinic-slide-03':'Consultation','clinic-slide-04':'Exam room','clinic-slide-05':'Clinic lounge','clinic-slide-06':'Fitness portrait'}[k])||'Supplied photo');b.setAttribute('aria-label',(PHOTO_LABELS[k]||photoNames[i]||({'clinic-slide-03':'Consultation','clinic-slide-04':'Exam room','clinic-slide-05':'Clinic lounge','clinic-slide-06':'Fitness portrait'}[k])||'Supplied photo'));const im=document.createElement('img');im.src=ASSETS[k];im.alt=(PHOTO_LABELS[k]||photoNames[i]||({'clinic-slide-03':'Consultation','clinic-slide-04':'Exam room','clinic-slide-05':'Clinic lounge','clinic-slide-06':'Fitness portrait'}[k])||'Supplied photo');b.append(im);b.onclick=()=>{if(!PHOTO_LAYOUTS.includes(active))select(['frame','focus'].includes(active)?'cream':'split');drafts[active].photoKey=k;drafts[active].custom=null;render()};$('photos').append(b)});ready=true;loadFields();render();$('export').disabled=false;$('compliance').disabled=false}catch(e){$('status').textContent='Could not load studio';console.error(e);toast('Reload the studio to load the included images.')}}init();

function filterPhotos(){const query=$('photoSearch').value.trim().toLowerCase();let count=0;document.querySelectorAll('#photos .photo').forEach(button=>{button.hidden=!button.title.toLowerCase().includes(query)&&!button.dataset.key.toLowerCase().includes(query);if(!button.hidden)count++});$('photoCount').textContent=count+' photos'+(query?' match your search':' available')}
$('photoSearch').oninput=filterPhotos;new MutationObserver(filterPhotos).observe($('photos'),{childList:true});
